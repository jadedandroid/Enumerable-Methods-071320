...
tiger_hash = zoos["Bronx Zoo"][:animals].find do |animal|
...
total_price = zoos.sum do |zoo, zoo_info|
    zoo_info[:price]
end
...
all_locations = zoos.map do |zoo, zoo_info|
    zoo_info[:location]
end
...
open_on_weekend = zoos.select do |zoo, zoo_info|
    zoo_info[:weekend]
end
...
first_zoo_open_on_weekend = zoos.find do |zoo, zoo_info|
...
zoo_with_monkeys = zoos.select do |zoo, zoo_info|
...
total_sum_of_animals = zoos.map do |zoo, zoo_info|
    total_number = zoo_info[:animals].map do |animal|
        animal[:count]# gets the count for each species in each zoo
    end
    total_number.sum# sum all the numbers per zoo and store in an array due to map
end
...
highest_species = zoos.max_by do |zoo, zoo_info|
    highest_per_zoo = zoo_info[:animals].max_by do |animal|
        animal[:count]
    end
    highest_per_zoo[:count]
end
